...
const Newmodule = require('./Newmodule');//imports Newmodule.js file functions

const directory = process.argv[2];//argument1 for directory
const extension = process.argv[3];//argument2 for extension

var callback = function(err, data){//callback function
  if(err){return err;}//error handling
  for (var i = 0; i < data.length; i++) {//printing data
    console.log(data[i]);
  }
}

Newmodule(directory, extension, callback);//calling Newmodule file


//Newmodule(directory, extension,(err, filteredList) => {
